...
const AddTaskModal: FC<any> = function ({ sharedState, updateSharedState }: any) {
  ...
  const url2 = `https://bn.glassmountainbpo.com:8080/inv/inventory/addBrand`;
...
const EditUserModal: FC<any> = function ({ id, active, name, sharedState, updateSharedState }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const response = await axios.post('https://bn.glassmountainbpo.com:8080/inv/inventory/editBrand', {
        id,
        nameCategory,
        status,
        created_user
